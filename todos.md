- VM Refactor
  ...
  - A switch to reference counting might solve the problem of having to run a periodic GC collect phase
    - Memory cells which are no longer used can now just be reclaimed as soon as they are no longer used
    - This could be as simple as just appending a pointer to the memory region to a free list
    - What disadvantages could this have?
      - Retain cycles
        - Imagine the following memory situation:
          ```
            A --> B --> C <-- D
            ^           |
            +-----------+
          ```
          Note how there is a cyclic reference between A, B and C.
          If we assume that D is a root and we remove that object, then ABC will be kept alive
          because we can't trivially figure out that their references are cyclical.
